...
import com.example.doantotnghiep.databinding.ActivityOrderProductAdminBinding

class OrderProductAdminActivity : AppCompatActivity(), IClickItem {
    lateinit var viewBinding : ActivityOrderProductAdminBinding
...
//    lateinit var
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewBinding = ActivityOrderProductAdminBinding.inflate(layoutInflater)
        setContentView(viewBinding.root)
        viewBinding.toolbar.setTitle("List Order")
        ...
        viewModelOrder.getListOrderUsers().observe(this, Observer {
            it?.apply {
                ...
                if (it.size == 0) {
                    ...
                    for (idUser in it) {
                        viewModelOrder.getListOrders(idUser).observe(this@OrderProductAdminActivity,
                            Observer {
                                it?.apply {
                                    Log.d("ORDERS", "${this.toString()}")
                                    for (data in it ) {
                                        Log.d("ORDERS", " ${data.idShop }- ${mData.id}")
                                    ...
                                    if (mListOrders.size == 0) {
                                        ...
                                        viewBinding.labelData.visibility = View.INVISIBLE
                                    }
                                    viewBinding.progressLoading.visibility = View.INVISIBLE
                                }
                            })
        ...
        viewBinding.recyclerOrder?.apply {
            adapter = orderAdapter
            layoutManager = LinearLayoutManager(this@OrderProductAdminActivity,LinearLayoutManager.VERTICAL,false)
            hasFixedSize()
        }
    }
